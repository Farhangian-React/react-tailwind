...
export default function CardDastiB({ itemsPerPage }) {
...
fetch('https://servers-nahall.onrender.com/Allproductsnahall' ).then((res)=>{
  ...
  setData(result.filter(i=>i.product === "kif" && i.type === "dastib"));
  setDatashow(result.filter(i=>i.product === "kif" && i.type === "dastib"));
  setIsLoading(false);
}).catch((err)=>{
