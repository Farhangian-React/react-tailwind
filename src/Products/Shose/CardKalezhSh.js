...
export default function CardKalezhSh({ itemsPerPage }) {
...
fetch('https://servers-nahall.onrender.com/Allproductsnahall' ).then((res)=>{
  ...
  setData(result.filter(i=>i.product === "shos" && i.type === "kalezh"));
  setDatashow(result.filter(i=>i.product === "shos" && i.type === "kalezh"));
  setIsLoading(false);
}).catch((err)=>{
