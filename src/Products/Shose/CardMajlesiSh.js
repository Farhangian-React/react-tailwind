...
export default function CardMajlesiSh({ itemsPerPage }) {
...
fetch('https://servers-nahall.onrender.com/Allproductsnahall' ).then((res)=>{
  ...
  setData(result.filter(i=>i.product === "shos" && i.type === "mjlesi"));
  setDatashow(result.filter(i=>i.product === "shos" && i.type === "mjlesi"));
  setIsLoading(false);
}).catch((err)=>{
