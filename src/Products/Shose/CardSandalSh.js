...
export default function CardSandalSh({ itemsPerPage }) {
...
fetch('https://servers-nahall.onrender.com/Allproductsnahall' ).then((res)=>{
  ...
  setData(result.filter(i=>i.product === "shos" && i.type === "sandal"));
  setDatashow(result.filter(i=>i.product === "shos" && i.type === "sandal"));
  setIsLoading(false);
}).catch((err)=>{
